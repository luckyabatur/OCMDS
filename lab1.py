...
def main():
    ...
    (V_arrow,) = ax1.plot(
        [X[0] + VX[0] * V_scale + RX_V[i] for i in range(len(RX_V))],
        [Y[0] + VY[0] * V_scale + RY_V[i] for i in range(len(RY_V))],
        color=V_color,
    )
    ...
    (W_arrow,) = ax1.plot(
        [X[0] + WX[0] * W_scale + RX_W[i] for i in range(len(RX_W))],
        [Y[0] + WY[0] * W_scale + RY_W[i] for i in range(len(RY_W))],
        color=W_color,
    )
    ...
    def animate(i):
        ...
        V_arrow.set_data(
            [X[i] + VX[i] * V_scale + RX_V[j] for j in range(len(RX_V))],
            [Y[i] + VY[i] * V_scale + RY_V[j] for j in range(len(RY_V))]
        )
        ...
        W_arrow.set_data(
            [X[i] + WX[i] * W_scale + RX_W[j] for j in range(len(RX_W))],
